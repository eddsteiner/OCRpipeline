    ...
    def load_cell(self, cell_value):
        ...
        try:
            if hasattr(self, "current_file_path"):
                parts = os.path.normpath(self.current_file_path).split(os.sep)
                # Extract components
                month = parts[1]
                dtype = parts[2]
                filename = parts[-1]

                # Extract table number from filename like february_max_1.csv
                table_num = filename.split('_')[-1].replace(".csv", "")
                table_dir = f"table_{table_num}"

                base_path = os.path.join("output", month, dtype, table_dir)
                img_path = os.path.join(base_path, f"row_{self.row_idx + 1}", f"col_{self.col_idx + 1}.png")
                ...
                print("⚠️ current_file_path not set")
        except Exception as e:
            print("⚠️ Error while loading image:", e)





    def load_from_correction_list(self, correction_csv_path):
        self.correction_csv_path = correction_csv_path
        self.corrections_df = pd.read_csv(correction_csv_path, quoting=csv.QUOTE_ALL, dtype=str)

        if 'corrected' not in self.corrections_df.columns:
            self.corrections_df['corrected'] = 0
            # ✅ Save immediately so the CSV on disk gets updated
            self.corrections_df.to_csv(self.correction_csv_path, index=False)

        self.correction_idx = 0
        self.apply_next_correction()


    def apply_next_correction(self):
        """
        Applies the next correction target in the list.
        """
        while self.correction_idx < len(self.corrections_df):
            row = self.corrections_df.iloc[self.correction_idx]

            if str(row.get('corrected', 0)) == "1":
                self.correction_idx += 1
                continue

            file_path = os.path.normpath(str(row['file']))
            if not os.path.exists(file_path):
                messagebox.showerror("Missing File", f"CSV file not found:\n{file_path}")
                self.correction_idx += 1
                continue

            # Load new file if it's different from the currently loaded one
            if getattr(self, 'current_file_path', None) != file_path:
                try:
                    self.current_csv = pd.read_csv(file_path, header=None, dtype=str)
            ...
            try:
                self.row_idx = int(row['row']) - 1
                self.col_idx = int(row['col']) - 1
                self.load_cell(self.current_csv.iat[self.row_idx, self.col_idx])
            except Exception as e:
                messagebox.showerror("Error", f"Invalid row/col reference in error_log.csv (Row {self.correction_idx + 1}):\n\n{e}")
                self.correction_idx += 1
                continue

            return  # Stop and wait for user correction

        messagebox.showinfo("Done", "All corrections have been reviewed.")

    def confirm_cell(self):
        """
        Overridden to save correction and mark it in correction list.
        """
        value = self.current_text.get()
        self.current_csv.iat[self.row_idx, self.col_idx] = "" if value.strip().lower() in {"x", "nan"} else value

        # Save the current CSV immediately
        if hasattr(self, 'current_file_path'):
            pd.DataFrame(self.current_csv).to_csv(self.current_file_path, index=False, header=False)
            print(f"Saved {value} to {self.current_file_path}")


        # Mark as corrected if we're working from a correction list
        if hasattr(self, 'corrections_df') and self.correction_idx < len(self.corrections_df):
            ...
            if hasattr(self, 'correction_csv_path'):
                self.corrections_df.to_csv(self.correction_csv_path, index=False)
            self.correction_idx += 1
            self.apply_next_correction()
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        correction_csv_path = sys.argv[1]
        app.load_from_correction_list(correction_csv_path)
