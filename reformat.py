...
OUTPUT_FILE = 'master_temperature_and_precip_data.csv'
...
for month in month_to_number.keys():
    ...
    for var_type in ['max', 'min', 'precipitation']:
        ...
        for file_path in files:
            ...
            df_melted['date'] = df_melted.apply(lambda row: f"{int(row['month']):02d}-{row['day']}-{int(row['year'])}"if pd.notnull(row['year']) and pd.notnull(row['day']) else None,axis=1)
...
if all_data:
    ...
    # Drop invalid dates (e.g., Feb 30, etc.)
