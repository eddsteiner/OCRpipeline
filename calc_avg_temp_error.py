...
def compute_file_error(file_path):
    ...
    left_col = 32  # column 33
    right_col = 34  # column 34

    if df.shape[1] <= right_col:
        print(f"⏭️ Skipping {file_path} (does not have 34 columns)")
        return

    try:
        base = pd.to_numeric(df[left_col], errors='coerce')
        change = pd.to_numeric(df[right_col], errors='coerce')
        ...
        print(f"❌ Error comparing cols 33 and 34 in {file_path}: {e}")
